...
class TinkerOpenmmConverter:
    ...
    def write(self, xml_file, pdb_file, name_prefix):
        ...
        with open(pdb_file, 'w') as f:
            ...
            for b in self.xyz_bonds:
                ...
                f.write('{0: >5}'.format(b.bondfrom))
                f.write('{0: >5}'.format(b.bondto))
                f.write("\n")
            '''
            ...
            for b in self.xyz_bonds:
                if b.bondfrom in existing_bonds:
                    if b.bondto in existing_bonds[b.bondfrom]:
                        continue
                if b.bondto in existing_bonds:
                    if b.bondfrom in existing_bonds[b.bondto]:
                        continue
            ...
            for key in existing_bonds.keys():
                f.write('{0: <6}'.format("CONECT"))
                f.write('{0: >5}'.format(key))
                for value in existing_bonds[key]:
                    f.write('{0: >5}'.format(value))
                f.write("\n")
    ...
    def xml_make_residue(self):
        ...
        for b in self.xyz_bonds:
            ...
            #print(str(b.bondfrom)+" "+str(b.bondto))
            #bond.set("atomName1", str(self.xyz_atoms[int(b.bondfrom)-1].name))
           # bond.set("atomName2", str(self.xyz_atoms[int(b.bondto)-1].name))
    ...
    def xml_make_amoeba_multipole_force(self):
        ...
        for elem in self.prm_multipole_records:
            ...
            mp.set("kx", sign(int(elem.kx))+str(self.atom_type_prefix) + str(abs(int(elem.kx))))
            mp.set("kz", sign(int(elem.kz))+str(self.atom_type_prefix) + str(abs(int(elem.kz))))
            mp.set("c0", elem.c_0)
        ...
        for elem in self.prm_polarize_records:
            ...
            for i in range(0, len(elem.pgrp)):
                pol.set("pgrp" + str(i + 1), str(self.atom_type_prefix)+elem.pgrp[i])
